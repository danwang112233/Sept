	...
	if(argc != 4) 
	...
	else
		{
			...
			for(int i = ne_min; i < ne_max +1; ++i)
			{
				...
				fout << "dump core_displ all custom 20000 displcoredump id type f_AVE_ATOM[1] f_AVE_ATOM[2] 						f_AVE_ATOM[3]" << endl;
				...
				fout << "print \" electric field strength = " << exf << " .\"" << endl;
				fout <<"fix NPT all npt temp 10 10 0.1 x 0.0 0.0 0.1 y 0.0 0.0 0.1 xy 0.0 0.0 0.1 couple none" << endl;
				fout << "print \" electric field strength = " << exf << " .\"" << endl;
